...
z=String[]
for i in 8:9
    push!(z,"green-13-0$(x1[i])");
end
for i in 1:3
    push!(z,"green-13-$(x2[i])");
end
for i in 1:9
    push!(z,"green-14-0$(x1[i])");
end
for i in 1:3
    push!(z,"green-14-$(x2[i])");
end
for i in 1:9
    push!(z,"green0$(x1[i])");
end
...
@everywhere function calculation(file::String)
        ...
        @time result = kmeans(xfilter',100; maxiter=200, display=:iter);
        println("DOne!!!! $(file)")
        return (result,file)
end
...
v
save("green_dropoff_centers.jld","v",v)
